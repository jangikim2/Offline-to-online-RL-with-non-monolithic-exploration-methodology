...
class PEX(IQL):
    def __init__(self, critic, vf, policy, optimizer_ctor,
        ...
        ##### johnny
        self.critic_offline = copy.deepcopy(self.critic).to(DEFAULT_DEVICE)
        ...
        if ckpt_path is not None:
            ...
            ##### johnny
            self.critic_offline.load_state_dict(critic_state_dict)
    ...
    def select_action_vae(self, vae, vae_online, count_vae, count_vae_online, beta, observations, evaluate=False, return_all_actions=False):
        observations = observations.unsqueeze(0)
        a1 = self.policy_offline.act(observations, deterministic=True)
        ...
        if vae.elbo_estimator(observations, a1, beta) >= vae_online.elbo_estimator(observations, a2, beta):
            q1 = self.critic_offline.min(observations, a1)
            q2 = self.critic_offline.min(observations, a2)
        else:
            q1 = self.critic.min(observations, a1)
            q2 = self.critic.min(observations, a2)
        ...
        if not return_all_actions:
            #return action.squeeze(0)
            return action.squeeze(0).detach().cpu().numpy(), count_vae, count_vae_online, action
        else:
            return action.squeeze(0), a1.squeeze(0), a2.squeeze(0)
